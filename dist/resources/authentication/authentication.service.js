...
(function (AuthenticationService) {
    AuthenticationService.SignIn = async (req) => {
        try {
            ...
            if (checkUser) {
                const isPasswordValid = bcrypt.compare(req?.body?.password, checkUser?.password);
    ...
    AuthenticationService.ForgotPassword = async (req) => {
        try {
            const checkUser = await UserModel.CredentialModel.findOne({
                email: req?.body?.email,
            });
